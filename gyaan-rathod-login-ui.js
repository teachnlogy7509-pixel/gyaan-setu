(() => {
  ...
  let gyaan = null, rathod = null, mode = 'login';

  const $ = (id) => document.getElementById(id);
  const loadSupabase = () => import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');

  async function clients() {
    if (gyaan && rathod) return { gyaan, rathod };
    const { createClient } = await loadSupabase();
    gyaan = gyaan || createClient(GYAAN_URL, GYAAN_KEY, {
      auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: true }
    });
    rathod = rathod || createClient(RATHOD_URL, RATHOD_KEY, {
  ...
  function setStatus(message, ok = false) {
    ...
    node.style.color = ok ? '#65d39a' : '#ffb4b4';
  }

  function build() {
    if ($('gyaanRathodAuth')) return;
    const style = document.createElement('style');
    style.id = 'gyaanSetuAuthStyle';
    style.textContent = `
      ...
      #gyaanRathodAuth .gs-auth-logo{width:66px;height:66px;border-radius:18px;background:linear-gradient(135deg,#effff8,#ddf7ee);border:1px solid rgba(37,139,104,.45);display:grid;place-items:center;margin:0 auto 10px;color:#258b68;font-size:25px;font-weight:950;letter-spacing:-1px;box-shadow:0 0 28px rgba(37,139,104,.18)}
      #gyaanRathodAuth .gs-auth-title{text-align:center;font-size:24px;font-weight:950;margin:0;letter-spacing:.02em}
      ...
      #gyaanRathodAuth .gs-auth-tabs button.active{background:#258b68;color:#fff;box-shadow:0 8px 22px rgba(37,139,104,.25)}
      ...
      #gyaanRathodAuth .gs-primary{border:0;background:#258b68;color:#fff}
      ...
      #gyaanRathodAuth .gs-migrate{display:none;width:100%;border:1px solid rgba(99,102,241,.35);border-radius:12px;padding:10px;margin-top:9px;background:rgba(79,70,229,.12);color:#e0e7ff;font-size:10px;font-weight:800;cursor:pointer}
      #gyaanRathodAuth .gs-note{font-size:9px;line-height:1.45;color:#94a3b8;text-align:center;margin-top:8px}
      ...
      #gyaanRathodAuth small{display:block;text-align:center;min-height:24px;margin-top:9px;font-size:10px}
    `;
    ...
    backdrop.innerHTML = `
      <div class="gs-auth-card" role="dialog" aria-modal="true">
        ...
        <div class="gs-auth-logo">GS</div>
        <h1 class="gs-auth-title">GYAANSETU</h1>
        <p class="gs-auth-sub">Learn Together · NEET Prep & Live Battles</p>
        ...
        <div class="gs-field">
          ...
          <input id="gsAuthEmail" class="gs-input" type="email" autocomplete="email" placeholder="you@example.com">
        </div>

        <div class="gs-field">
          <label>Password</label>
          <input id="gsAuthPassword" class="gs-input" type="password" autocomplete="current-password" minlength="6" placeholder="Minimum 6 characters">
        </div>

        <button id="gsAuthSubmit" class="gs-primary" type="button">Login</button>
        <button id="gsMigrate" class="gs-migrate" type="button">🔄 RATHOD HUB account mila — GyaanSetu mein same account enable करें</button>
        ...
        <div class="gs-note">GyaanSetu login uses its own Supabase Auth. Existing RATHOD HUB accounts can be migrated without changing their RATHOD HUB account.</div>
      </div>`;
    document.body.appendChild(backdrop);

    const open = () => { backdrop.classList.add('show'); setMode('login'); };
    ...
    if (account && !account.dataset.gsAuthBound) {
      ...
      account.addEventListener('click', async (e) => {
        ...
        const { gyaan: c } = await clients();
        const current = (await c.auth.getSession()).data?.session;
        if (current && typeof window.openProfile === 'function') {
          try { await window.openProfile(); return; } catch (_) {}
        }
        open();
      }, true);
    }
    ...
    function setMode(next) {
      ...
      $('gsMigrate').style.display = next === 'login' ? 'none' : 'none';
      setStatus('');
    }
    ...
    async function submit() {
      ...
      try {
        ...
        if (mode === 'signup') {
          ...
          if (!login.data?.session) throw new Error('Session create नहीं हुआ. फिर से Login करें.');
          setStatus('GyaanSetu account created ✓', true);
          close();
  ...
  async function init() {
    try {
      ...
      console.error('GyaanSetu authentication UI failed:', error);
    }
  }
